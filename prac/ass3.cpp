...
string permute(string k,int *a,int n){
	string per="";
	for(int i=0;i<n;i++){
		per+=k[a[i]-1];
	}
...
//xor
// A XOR B = 0 when A = 0 and B = 0 or when A = 1 and B = 1.
// A XOR B = 1 when A = 0 and B = 1 or when A = 1 and B = 0.
...
string encrypt(string plain, vector<string> k_bin, vector<string> k_hex){
	plain=hex2bin(plain);
 ...
 //p-box
	...
	int s[8][4][16] = { 
 	...
 	for(int i=0;i<16;i++){
 		...
 		//xor round_key and right expanded
 		string x=xor_op(k_bin[i],right_expanded);
 		...
 		cout << "Round " << i + 1 << " " << bin2hex(left) << " " << bin2hex(right) << " " << k_hex[i] << endl; 
 	}
...
//for key-p we generate the table using the initial permutation and removing the (64, 56, 48, 40, 32, 24, 16, 8,) bits and then arrange them in sorting order and then after 36 place the last row and similarly other rows
				// 57, 49, 41, 33, 25, 17, 9, 1, 
				// 58, 50, 42, 34, 26, 18, 10, 2, 
				// 59, 51, 43, 35, 27, 19, 11, 3, 
				// 60, 52, 44, 36, 28, 20, 12, 4, 
...
//s-box denge p-box or comp vala yaad karna h